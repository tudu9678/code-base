...
docker exec -i "$master_db" psql -U postgres -d postgres -c "ALTER SYSTEM SET wal_level TO 'logical';"
docker exec -i "$master_db" psql -U postgres -d postgres -c "SELECT pg_reload_conf();"
## IMPORTANT : calling pg_reload_conf() function for config reload or db restart
docker restart logrepl_pg_master
sleep 8
# Configure the master database
echo "Configuring $master_db as master..."
